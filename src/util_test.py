...
class CircuitTest(unittest.TestCase):
    def noise_conf_for_testing(self) -> NoiseConfiguration:
        return NoiseConfiguration(
            single_qubit_gate_error_probability=0.01,
            two_qubit_gate_error_probability=0.02,
            reset_error_probability=0.03,
            measurement_error_probability=0.04,
            idle_error_probability=0.05)

    def test_place_single_qubit_gate(self):
        mapping = QubitMapping(20, 20)
        noise_conf = self.noise_conf_for_testing()
